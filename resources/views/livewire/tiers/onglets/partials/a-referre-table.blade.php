<table class="table table-sm mb-0">
    ...
    @foreach($lignes as $ligne)
        <tr wire:key="a-referre-{{ $ligne->participant->id }}"
            onclick="if (!event.target.closest('button,a,.no-row-click')) window.location='{{ route('operations.show', $ligne->operationId()) }}'"
            style="cursor:pointer">
            <td>
                <a href="{{ route('tiers.show', $ligne->tiersReferreId()) }}" class="no-row-click text-decoration-none">{{ $ligne->tiersReferreNomComplet() }}</a>
            </td>
            <td>
