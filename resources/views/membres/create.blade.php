            ...
            <form action="{{ route('membres.store') }}" method="POST">
                ...
                <div class="row mb-3">
                    ...
                    <div class="col-md-6">
                        ...
                        <select name="statut_membre" id="statut_membre" class="form-select @error('statut_membre') is-invalid @enderror" required>
                            <option value="actif" {{ old('statut_membre', 'actif') === 'actif' ? 'selected' : '' }}>Actif</option>
                            <option value="inactif" {{ old('statut_membre', 'actif') === 'inactif' ? 'selected' : '' }}>Inactif</option>
                        </select>
                        @error('statut_membre')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="mb-3">
                    ...
                    <textarea name="notes_membre" id="notes_membre" class="form-control @error('notes_membre') is-invalid @enderror"
                              rows="2">{{ old('notes_membre') }}</textarea>
                    @error('notes_membre')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
