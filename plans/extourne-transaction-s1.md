...
- [ ] Compte de résultat : exercice avec recette +80 € + extourne -80 € pointées affiche ∑ produits sous-cat = 0 €
- [ ] Flux trésorerie : extourne pointée à un rapprochement bancaire ordinaire affecte le solde de trésorerie
- [ ] Multi-tenant : aucune fuite — comptable de tenant A ne peut pas extourner une transaction du tenant B (scope global retourne null, service refuse)
...
- [ ] Cohérence atomique flag `transactions.extournee_at` ↔ entrée `extournes` : crash artificiel mid-transaction → rollback propre, ni flag ni entrée subsistent
...
- [ ] Eager loading sur listes pour éviter N+1 sur le badge "annulée" (test mesurant queries sur 25 transactions extournées)
...
- `app/Services/TransactionExtourneService.php` : `final class`, méthode `extourner(Transaction $origine, ExtournePayload $payload): Extourne`
  - `DB::transaction(function () { … })` :
    ...
    8. `event(new TransactionExtournee($extourne))`
    9. Return `$extourne`
...
- **Test multi-tenant intrusion** : crée tx du tenant B, boote tenant A, appel `extourner` avec ID de B → soit `Transaction::find()` retourne null (scope global), soit `RuntimeException`. Asserter qu'aucune Extourne créée en base, aucun flag set (vérifier état tenant B intact via `withoutTenantScope()` ou tableau global)
...
- Test `crash_avant_creation_extourne_rollback_propre` : forcer une exception après création de la transaction miroir mais avant l'entrée Extourne (mock event ou throw dans la fonction `creerLettrage`) → asserter aucune Transaction miroir, aucune Extourne, `origine.extournee_at` reste null, aucun RapprochementBancaire lettrage
- Test `crash_apres_set_extournee_at_rollback_flag` : forcer exception après `$origine->save()` du flag → asserter rollback du flag également
- Stratégie : injecter un closure qui throw, ou utiliser un partial mock du service

**GREEN**: Si tout est bien dans `DB::transaction()` (Step 5/6), tests doivent déjà passer. Sinon corriger l'enrobage transactionnel.
...
- Test `extourne_en_attente_cas_encaissé_n_apparait_pas_dans_creances` : tx recette `Recu`, extourne via service → extourne `EnAttente` à -80 € → vue Créances ne l'inclut pas (S0 a déjà ajouté filtre `montant > 0` selon mémoire — vérifier que ce filtre couvre bien le cas, sinon étendre)
...
- **Test eager loading N+1** : créer 25 transactions extournées + lister via `TransactionUniverselle` → mesurer queries via `DB::enableQueryLog()` → asserter ≤ 5 queries (pas 25 + N)
...
- Test `compte_de_resultat_recette_nette_zero` : exercice 2026, recette +80 € pointée + extourne -80 € pointée même sous-cat → CompteResultatBuilder retourne `produit_total_souscat = 0 €`, détail liste les 2 écritures séparément
- Test `flux_tresorerie_extourne_pointee_banque_ordinaire_affecte_solde` : extourne pointée à un rapprochement bancaire ordinaire (cas encaissé) → solde de trésorerie reflète le débit
- Test `flux_tresorerie_lettrage_neutre` : lettrage (cas non-encaissé) → solde de trésorerie inchangé (car aucun mouvement bancaire réel)
- Test `journal_transactions_export_excel_montre_extourne_avec_signe` : export sur exercice avec extourne → vérifier signe explicite et somme nette correcte
