# Plan: Facture libre — Slice 2 (transformation devis → facture, lignes libres, génération transaction)
...
Étendre le module Facture pour qu'il porte trois types de lignes (`Montant` ref, `MontantLibre` libre, `Texte` info) et qu'à la validation d'une facture portant des lignes libres, **une `Transaction` recette + N `TransactionLignes`** soient générées automatiquement (statut "à recevoir", mode = `facture.mode_paiement_prevu`). Permettre la transformation d'un devis libre accepté en facture brouillon, et la création d'une facture libre directe sans devis source. Embarquer le fix d'un bug PDF préexistant sur les lignes `Texte` (devis + facture). Pivot architectural invoice-first, traçé dans un ADR-002.

## Acceptance Criteria

- [ ] Un devis libre `accepté` peut être transformé en facture brouillon par bouton dédié, lignes recopiées (`MontantLibre`/`Texte`), `factures.devis_id` renseigné. Bouton désactivé après transformation.
- [ ] Une facture libre vierge peut être créée directement depuis la liste, sans devis source.
- [ ] L'éditeur de facture brouillon permet d'ajouter des lignes des trois types (`Montant` ref via picker, `MontantLibre` libre, `Texte` info), avec recalcul live du total.
- [ ] Le bloc "Conditions de règlement" expose un champ `mode_paiement_prevu` (énuméré `ModePaiement`), visible et requis ssi la facture porte ≥ 1 ligne `MontantLibre`.
- [ ] À la validation d'une facture portant ≥ 1 ligne `MontantLibre` : guards `mode_paiement_prevu` requis + `sous_categorie_id` requise sur chaque `MontantLibre` ; **1 `Transaction` recette + N `TransactionLignes`** créées dans une transaction DB ; `facture_lignes.transaction_ligne_id` set sur chaque ligne libre ; pivot `facture_transaction` mis à jour.
...
- [ ] Les transactions générées par les lignes libres sont verrouillées par la facture validée (`isLockedByFacture()` existant).
- [ ] PDF facture : option α appliquée — `Montant` ref affiche libellé+montant total (PU/Qté vides), `MontantLibre` affiche libellé+PU+Qté+montant total, `Texte` affiche libellé seul (PU/Qté/Montant vides).
- [ ] PDF devis libre : lignes `Texte` n'affichent plus `0,00 €` dans les colonnes PU/Qté/Montant (bug fix embarqué).
- [ ] Multi-tenant : facture libre + transaction générée invisibles depuis une autre association — assertions sur la liste, la vue 360°, la recherche, et l'accès direct par ID (HTTP 403 ou 404).
...
### Step 2: Enum `TypeLigneFacture` — 3e valeur `MontantLibre` + helpers

**Complexity**: trivial
**RED**: test enum unitaire — 3 cases présentes (`Montant`, `MontantLibre`, `Texte`) ; `genereTransactionLigne()` true uniquement pour `MontantLibre` ; `aImpactComptable()` true pour `Montant` et `MontantLibre`, false pour `Texte`.
**GREEN**: ajouter `case MontantLibre = 'montant_libre';` + 2 méthodes helper.
**REFACTOR**: None needed.
**Files**: `app/Enums/TypeLigneFacture.php`, `tests/Unit/Enums/TypeLigneFactureTest.php`
**Commit**: `feat(facture-libre): enum TypeLigneFacture étendu (MontantLibre + helpers)`
...
**Commit**: `feat(facture-libre): modèles Facture/FactureLigne/Devis enrichis (devis_id, mode_paiement_prevu, lignes libres)`

### Step 4: `FactureService::creerLibreVierge` — création facture libre directe
...
**Commit**: `feat(facture-libre): FactureService::creerLibreVierge (facture libre directe brouillon)`

### Step 5: `FactureService::ajouterLigneLibre[Montant|Texte]` + recalcul total

**Complexity**: standard
**RED**: tests :
- `ajouterLigneLibreMontant(facture, ['libelle'=>..., 'prix_unitaire'=>800, 'quantite'=>3, 'sous_categorie_id'=>...])` crée FactureLigne `MontantLibre`, `montant = 2400`, `transaction_ligne_id = null`, `operation_id = null`, `seance = null` (sauf si fournis), ordre incrémental, `Facture.montant_total` recalculé
...
- facture avec ≥ 1 `MontantLibre` et `mode_paiement_prevu = null` → exception ciblée à la validation
- facture avec ≥ 1 `MontantLibre` sans `sous_categorie_id` → exception ciblée
- facture sans `MontantLibre` (que des `Montant` ref ou `Texte`) → validation OK même si `mode_paiement_prevu = null` (régression : les factures classiques continuent à fonctionner)
...
- 1 facture brouillon, 2 `MontantLibre` 1200€/200€ + mode "virement" → 1 Transaction recette créée (montant_total=1400, statut_reglement="à recevoir", mode_paiement="virement", `libelle = "Facture {$facture->numero}"` où `$numero` est attribué dans la même `DB::transaction` AVANT la génération), 2 TransactionLignes (montants/sous_cat/operation/seance recopiés, notes=libellé facture_ligne), pivot `facture_transaction` peuplé, chaque facture_ligne libre a son `transaction_ligne_id` setté
...
- mix `Montant` ref + `MontantLibre` : pivot porte les transactions ref + la nouvelle, total facture cohérent
- facture sans `MontantLibre` → aucune nouvelle transaction créée
...
**GREEN**: dans `valider()`, après guards et attribution numéro, si lignes `MontantLibre` existent :
...
2. Pour chaque `MontantLibre` : créer `TransactionLigne` (transaction_id, sous_cat, operation, seance, montant, notes=libelle)
3. Update `facture_lignes.transaction_ligne_id` sur chaque ligne libre
...
- devis "accepté" avec 2 lignes montant + 1 ligne texte → `Facture` brouillon créée, `tiers_id` cohérent, `devis_id = devis.id`, lignes recopiées avec **conversion explicite de type** : `DevisLigne` type `Montant` → `FactureLigne` type **`MontantLibre`** (assertion `factureLigne->type === TypeLigneFacture::MontantLibre`), `DevisLigne` type `Texte` → `FactureLigne` type `Texte`, montant_total cohérent
...
### Step 10: UI FactureList — bouton "Nouvelle facture libre"

**Complexity**: standard
**RED**: test Livewire `FactureList` — bouton "Nouvelle facture libre" présent à côté de "Nouvelle facture" ; ouvre modale `TiersAutocomplete` ; sélection tiers → appelle `FactureService::creerLibreVierge` + redirige vers FactureEdit du brouillon.
...
**Commit**: `feat(facture-libre): bouton 'Nouvelle facture libre' sur FactureList`
...
- éditeur permet d'ajouter une ligne `MontantLibre` (form libellé/PU/qté/sous_cat/opération/séance) → service appelé, total recalculé live
- éditeur permet d'ajouter une ligne `Texte` (libellé seul) → service appelé
- bloc "Conditions de règlement" affiche `mode_paiement_prevu` (select `ModePaiement`) ssi la facture porte ≥ 1 `MontantLibre` ; persiste sur changement
...
**GREEN**: étendre `FactureEdit` Livewire + blade. Réutiliser pattern de saisie de ligne existant pour `Montant` ; ajouter section dédiée pour `MontantLibre`/`Texte`.
...
### Step 12: PDF devis libre — fix bug lignes `Texte`
...
- ligne `MontantLibre` → libellé + PU + Qté + montant total rendus
...
- intrusion : facture libre + transaction générée invisibles depuis une autre association (liste, recherche, vue 360°, accès direct par ID)
...
**Complexity**: standard
**RED**: aucun test code. Critères : ADR rédigé, CHANGELOG mis à jour, seeders de démo permettent de voir la fonctionnalité en local (≥ 1 devis transformé en facture, ≥ 1 facture libre directe validée avec transaction générée, ≥ 1 facture libre brouillon).
**GREEN**: 
...
- ticket dette "Avoir n'annule pas transactions issues de lignes libres" créé (issue GitHub)
**REFACTOR**: None needed.
...
- [ ] Recette manuelle parcours facture libre directe → validation → encaissement
...
- **Couplage `FactureEdit` existant** : la blade actuelle peut nécessiter une refonte non-triviale pour porter 3 types de lignes. Mitigation : Step 11 classé `complex`, prévoir partials par type ; au pire, splitter le step en 11a (saisie MontantLibre/Texte) + 11b (mode_paiement_prevu).
