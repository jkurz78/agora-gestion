...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save') {
        ...
        $db->beginTransaction();
        try {
            if (empty($_POST['recette_id'])) {
                $stmt = $db->prepare('INSERT INTO recettes (date,libelle,montant_total,mode_paiement,payeur,reference,compte_id,notes,saisi_par) VALUES (?,?,?,?,?,?,?,?,?)');
                $stmt->execute([
                    $_POST['date'],
                    trim($_POST['libelle']),
                    $montant_total,
                    $_POST['mode_paiement'],
                    trim($_POST['payeur']) ?: null,
                    trim($_POST['reference'])    ?: null,
                    $_POST['compte_id']          ?: null,
                    trim($_POST['notes'])         ?: null,
                    $_SESSION['user_id'],
                ]);
                $recette_id = (int)$db->lastInsertId();
                $msg = 'Recette créée.';
            } else {
                $recette_id = (int)$_POST['recette_id'];
                $db->prepare('UPDATE recettes SET date=?,libelle=?,montant_total=?,mode_paiement=?,payeur=?,reference=?,compte_id=?,notes=? WHERE id=?')
                   ->execute([
                       $_POST['date'],
                       trim($_POST['libelle']),
                       $montant_total,
                ...
                $db->prepare('DELETE FROM recette_lignes WHERE recette_id=?')->execute([$recette_id]);
                $msg = 'Recette mise à jour.';
            }
            ...
            $ins = $db->prepare('INSERT INTO recette_lignes (recette_id,sous_categorie_id,operation_id,seance,montant,notes) VALUES (?,?,?,?,?,?)');
        ...
        header('Location: /accounting/pages/recettes.php'); exit;
    }

    if ($action === 'delete') {
        $db->prepare('DELETE FROM recettes WHERE id=?')->execute([(int)$_POST['id']]);
...
if ($edit_id) {
    $s = $db->prepare('SELECT * FROM recettes WHERE id=?');
    $s->execute([$edit_id]);
    $edit_rec = $s->fetch();
    if ($edit_rec) {
        $s2 = $db->prepare('SELECT * FROM recette_lignes WHERE recette_id=? ORDER BY id');
        $s2->execute([$edit_id]);
...
<div class="d-flex justify-content-between align-items-center mb-4">
  <h2><?= $edit_rec ? 'Modifier la recette' : 'Nouvelle recette' ?></h2>
  <a href="/accounting/pages/recettes.php" class="btn btn-outline-secondary">← Retour</a>
</div>

<form method="post" id="recetteForm">
  <input type="hidden" name="csrf_token"  value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
  <input type="hidden" name="action"      value="save">
  <input type="hidden" name="recette_id"  value="<?= $edit_id ?: '' ?>">

  <!-- Header -->
  <div class="card shadow-sm mb-4">
    <div class="card-header fw-semibold">En-tête de la recette</div>
    <div class="card-body">
      <div class="row g-3">
        <div class="col-md-2">
          <label class="form-label">Date <span class="text-danger">*</span></label>
          <input type="date" name="date" class="form-control" required
                 value="<?= htmlspecialchars($edit_rec['date'] ?? date('Y-m-d')) ?>">
        </div>
        <div class="col-md-4">
          <label class="form-label">Libellé <span class="text-danger">*</span></label>
          <input type="text" name="libelle" class="form-control" required maxlength="255"
                 value="<?= htmlspecialchars($edit_rec['libelle'] ?? '') ?>">
        </div>
        <div class="col-md-2">
          ...
          <select name="mode_paiement" class="form-select" required>
            ...
            $cur_mode = $edit_rec['mode_paiement'] ?? 'virement';
        ...
        <div class="col-md-2">
          ...
          <input type="text" name="payeur" class="form-control" maxlength="150"
                 value="<?= htmlspecialchars($edit_rec['payeur'] ?? '') ?>">
        </div>
        <div class="col-md-2">
          <label class="form-label">Réf. / N° pièce</label>
          <input type="text" name="reference" class="form-control" maxlength="100"
                 value="<?= htmlspecialchars($edit_rec['reference'] ?? '') ?>">
        </div>
        <div class="col-md-3">
          ...
          <select name="compte_id" class="form-select">
            ...
            <?php foreach ($comptes as $c): ?>
              <option value="<?= (int)$c['id'] ?>" <?= ($edit_rec['compte_id'] ?? '') == $c['id'] ? 'selected' : '' ?>>
        ...
        <div class="col-md-9">
          ...
          <input type="text" name="notes" class="form-control" maxlength="500"
                 value="<?= htmlspecialchars($edit_rec['notes'] ?? '') ?>">
        </div>
      </div>
    </div>
  </div>
  ...
  <div class="d-flex gap-2">
    ...
    <a href="/accounting/pages/recettes.php" class="btn btn-secondary">Annuler</a>
  </div>
</form>
...
<div class="d-flex justify-content-between align-items-center mb-4">
  <h2 class="mb-0">Recettes</h2>
  <a href="?new" class="btn btn-primary"><i class="bi bi-plus-lg"></i> Nouvelle recette</a>
</div>
...
<form method="get" class="row g-2 mb-3 align-items-end">
  ...
  <div class="col-sm-1">
    <a href="/accounting/pages/recettes.php" class="btn btn-sm btn-outline-secondary w-100">Reset</a>
  </div>
</form>
...
$where  = ['1=1'];
$params = [];
if (!empty($_GET['date_debut'])) { $where[] = 'r.date >= ?'; $params[] = $_GET['date_debut']; }
if (!empty($_GET['date_fin']))   { $where[] = 'r.date <= ?'; $params[] = $_GET['date_fin']; }
if (!empty($_GET['compte_id']))  { $where[] = 'r.compte_id = ?'; $params[] = (int)$_GET['compte_id']; }
if (isset($_GET['pointe']) && $_GET['pointe'] !== '') { $where[] = 'r.pointe = ?'; $params[] = (int)$_GET['pointe']; }

$stmt = $db->prepare('
    SELECT r.*, u.nom AS saisi_nom, cb.nom AS compte_nom
    FROM recettes r
    LEFT JOIN users u ON u.id = r.saisi_par
    LEFT JOIN comptes_bancaires cb ON cb.id = r.compte_id
    WHERE ' . implode(' AND ', $where) . '
    ORDER BY r.date DESC, r.created_at DESC
');
$stmt->execute($params);
$recettes = $stmt->fetchAll();
$total_filtre = array_sum(array_column($recettes, 'montant_total'));
$modes_labels = ['virement' => 'Virement', 'cheque' => 'Chèque', 'especes' => 'Espèces', 'cb' => 'CB', 'prelevement' => 'Prélèvement'];
?>

<div class="card shadow-sm">
  <div class="card-header d-flex justify-content-between align-items-center">
    <span><?= count($recettes) ?> recette(s)</span>
  ...
  <div class="card-body p-0">
    <table class="table table-hover mb-0">
      <thead class="table-light">
        <tr>
          <th>Date</th><th>Libellé</th><th>Payeur</th><th>Mode</th>
      ...
      <?php foreach ($recettes as $r): ?>
        <tr>
          ...
          <td>
            <span class="fw-medium"><?= htmlspecialchars($r['libelle']) ?></span>
            <?php if ($r['reference']): ?><br><span class="text-muted small"><?= htmlspecialchars($r['reference']) ?></span><?php endif; ?>
          </td>
          <td class="small"><?= htmlspecialchars($r['payeur'] ?? '—') ?></td>
          <td class="small"><?= $modes_labels[$r['mode_paiement']] ?? htmlspecialchars($r['mode_paiement']) ?></td>
          <td class="small"><?= htmlspecialchars($r['compte_nom'] ?? '—') ?></td>
          <td class="text-end fw-medium text-success"><?= number_format((float)$r['montant_total'], 2, ',', ' ') ?> €</td>
          <td><?= $r['pointe'] ? '<span class="text-success">✓</span>' : '' ?></td>
          <td class="text-end text-nowrap">
            <a href="?edit=<?= (int)$r['id'] ?>" class="btn btn-sm btn-outline-secondary me-1">Modifier</a>
            <form method="post" class="d-inline" onsubmit="return confirm('Supprimer cette recette ?')">
              <input type="hidden" name="csrf_token" value="<?= htmlspecialchars($_SESSION['csrf_token']) ?>">
      ...
      <?php endforeach; ?>
      <?php if (!$recettes): ?>
        <tr><td colspan="8" class="text-muted text-center py-3">Aucune recette trouvée.</td></tr>
      <?php endif; ?>
      </tbody>
    </table>
  </div>
