...
/**
 * Crée une dette fournisseur T1 (607 D / 401 C) via pourDepenseACredit.
 */
...
// ---------------------------------------------------------------------------
// Cas 7 : Règlement chèque émis → T2 : 401 D X (tiers) / 512 C X (tiers) + auto-lettrage
// ---------------------------------------------------------------------------
test('pourReglementFournisseur chèque crée T2 401 D (tiers) / 512 C (tiers) + auto-lettrage', function () {
    ...
    // Ligne 512 : crédit avec tiers (chèque émis → 512X directement)
    ...
    expect((int) $ligne512T2->tiers_id)->toBe((int) $tiers->id);
...
// ---------------------------------------------------------------------------
// Cas 8 : Règlement espèces → T2 : 401 D X (tiers) / 530 C X (tiers) + auto-lettrage
// ---------------------------------------------------------------------------
test('pourReglementFournisseur espèces crée T2 401 D (tiers) / 530 C (tiers) + auto-lettrage', function () {
...
// ---------------------------------------------------------------------------
// Cas 9 : Règlement virement → T2 : 401 D X (tiers) / 512BNP C X (tiers) + auto-lettrage
// ---------------------------------------------------------------------------
test('pourReglementFournisseur virement crée T2 401 D (tiers) / 512BNP C (tiers) + auto-lettrage', function () {
...
// ---------------------------------------------------------------------------
// Cas 10 : Règlement CB → T2 : 401 D / 512 C (tiers) + auto-lettrage
// ---------------------------------------------------------------------------
test('pourReglementFournisseur CB crée T2 401 D (tiers) / 512 C (tiers) + auto-lettrage', function () {
    ...
    // Auto-lettrage
    expect($ligne401T1->fresh()->lettrage_code)->toBe($t2->lignes->firstWhere('compte_id', $compte401->id)->fresh()->lettrage_code);
});
...
test('pourReglementFournisseur : solde ouvert 401 du tiers = 0 après règlement', function () {
    ...
    // Avant règlement : solde = -175 (dette)
...
// ---------------------------------------------------------------------------
// Cas 12 : Audit lettrage créé : ligne `lettrage_audit` action='lettre' avec IDs 401-T1 + 401-T2
// ---------------------------------------------------------------------------
...
test('pourReglementFournisseur lève LettrageDejaPresentException si dette source déjà lettrée', function () {
    ...
    // Simuler une dette déjà réglée (lettrage manuel)
