...
{
    return $service->creerLibreVierge($tiersId);
}

/**
 * Ajoute une ligne MontantLibre à la facture (peut avoir sous_categorie_id null ou fourni).
 */
function ajouterLigneMontantLibre(
    Facture $facture,
    int $sousCategId,
...
// ─── Guard 1 : mode_paiement_prevu requis quand ≥ 1 MontantLibre ────────────

describe('Guard mode_paiement_prevu requis sur facture MontantLibre', function () {

    it('lève une exception si mode_paiement_prevu est null et la facture porte une ligne MontantLibre', function () {
        $facture = creerFactureBrouillon($this->service, $this->tiers->id);

        // Ajouter une ligne MontantLibre avec sous_cat (donc guard sous_cat ne bloquera pas)
        ajouterLigneMontantLibre($facture, $this->sousCategorie->id, avecSousCat: true);
...
// ─── Guard 2 : sous_categorie_id requise sur chaque MontantLibre ─────────────

describe('Guard sous_categorie_id requise sur chaque ligne MontantLibre', function () {

    it('lève une exception si une ligne MontantLibre n\'a pas de sous_categorie_id (mode_prevu fourni)', function () {
        $facture = creerFactureBrouillon($this->service, $this->tiers->id);

        // Ajouter une ligne MontantLibre SANS sous_cat
        ajouterLigneMontantLibre($facture, $this->sousCategorie->id, avecSousCat: false);
    ...
    it('lève une exception si 2 lignes MontantLibre dont 1 sans sous_categorie_id (mode_prevu fourni)', function () {
        $facture = creerFactureBrouillon($this->service, $this->tiers->id);
...
// ─── Happy path : facture MontantLibre complète → statut passe à Validee ────

describe('Happy path : guards passent (mode_prevu + sous_cat fournis)', function () {

    it('facture brouillon avec 1 MontantLibre complet + mode_prevu fourni → statut passe à Validee, aucune exception', function () {
        $facture = creerFactureBrouillon($this->service, $this->tiers->id);

        // Ajouter une ligne MontantLibre complète
        ajouterLigneMontantLibre($facture, $this->sousCategorie->id, avecSousCat: true);
