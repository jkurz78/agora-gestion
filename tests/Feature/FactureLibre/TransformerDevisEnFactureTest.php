...
describe('Happy path : devis accepté (2 lignes Montant + 1 Texte) → Facture brouillon', function () {
    ...
    it('les deux lignes Montant du devis sont converties en MontantLibre sur la facture', function () {
        $devis = creerDevisAccepte($this->tiers, $this->sousCategorie);

        $facture = $this->service->transformerEnFacture($devis);

        $lignesMontantLibre = $facture->lignes()
            ->where('type', TypeLigneFacture::MontantLibre->value)
            ->get();

        expect($lignesMontantLibre)->toHaveCount(2);

        foreach ($lignesMontantLibre as $ligne) {
    ...
    it('les libelle, prix_unitaire, quantite et sous_categorie_id sont recopiés sur les lignes MontantLibre', function () {
        $devis = creerDevisAccepte($this->tiers, $this->sousCategorie);
        ...
        $premiere = $lignesMontantLibre->first();
        ...
        $seconde = $lignesMontantLibre->skip(1)->first();
    ...
    it('l\'ordre des lignes est préservé', function () {
        ...
        // Première ligne = Mission audit (type MontantLibre)
        expect($lignesOrdered->first()->libelle)->toBe('Mission audit')
            ->and($lignesOrdered->first()->type)->toBe(TypeLigneFacture::MontantLibre);
    ...
    it('persiste la facture et les lignes en base de données', function () {
        ...
        $this->assertDatabaseHas('facture_lignes', [
            ...
            'type' => TypeLigneFacture::MontantLibre->value,
            'libelle' => 'Mission audit',
        ]);
