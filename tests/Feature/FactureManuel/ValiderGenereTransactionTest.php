...
// ─── Helper : facture brouillon avec N lignes MontantManuel ──────────────────
...
function creerFactureLibre(
    ...
    $facture = $service->creerManuelleVierge($tiersId);
...
/**
 * Ajoute une ligne MontantManuel directement en DB (sans passer par le service).
 */
...
describe('Happy path : 2 lignes MontantManuel → 1 Transaction recette + 2 TransactionLignes', function () {
...
// ─── Test 2 : Mix Montant ref + MontantManuel ────────────────────────────────

describe('Mix Montant (ref) + MontantManuel → 2 transactions dans le pivot', function () {

    it('crée 1 NOUVELLE Transaction pour les lignes manuelles + conserve la ref dans le pivot', function () {
    ...
    it('le total facture reste 700 après validation (Montant ref 500 + MontantManuel 200)', function () {
        $transactionExistante = Transaction::factory()->asRecette()->create([
            'tiers_id' => $this->tiers->id,
            'montant_total' => 500.0,
        ]);
...
// ─── Test 3 : Facture sans MontantManuel → aucune Transaction créée ───────────

describe('Facture sans MontantManuel → aucune nouvelle Transaction créée', function () {
    ...
    it('validation d\'une facture classique (Montant ref uniquement) ne crée pas de Transaction', function () {
        ...
        // Utilise la méthode creer classique (pas creerManuelleVierge)
    ...
    it('validation d\'une facture libre avec lignes Montant ref + Texte uniquement ne crée pas de Transaction', function () {
        ...
        // Ligne Texte uniquement (pas de MontantManuel)
        $this->service->ajouterLigneTexteManuelle($facture, 'Mention contractuelle');
        $facture->refresh();
...
describe('Logs : facture.valide émis avec facture_id + transaction_id_generee', function () {
    ...
    it('émet facture.valide avec transaction_id_generee null pour une facture sans MontantManuel', function () {
        $transactionExistante = Transaction::factory()->asRecette()->create([
            'tiers_id' => $this->tiers->id,
            'montant_total' => 400.0,
        ]);
