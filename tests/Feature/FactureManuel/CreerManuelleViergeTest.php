...
describe('creerManuelleVierge()', function () {
    it('crée une facture brouillon vierge avec les bons attributs', function () {
        $facture = $this->service->creerManuelleVierge($this->tiers->id);
    ...
    it('refuse un tiers appartenant à une autre association (guard multi-tenant)', function () {
        ...
        expect(fn () => $this->service->creerManuelleVierge($tiersCrosstenant->id))
    ...
    it('échoue si TenantContext non booté (fail-closed)', function () {
        ...
        expect(fn () => $this->service->creerManuelleVierge($this->tiers->id))
            ->toThrow(RuntimeException::class);
    });

    it('crée deux factures distinctes sur deux appels successifs (idempotence)', function () {
        $facture1 = $this->service->creerManuelleVierge($this->tiers->id);
        $facture2 = $this->service->creerManuelleVierge($this->tiers->id);
