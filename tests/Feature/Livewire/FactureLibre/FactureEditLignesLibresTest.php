...
// ── Test 1 : Bouton "Ajouter ligne libre" présent ssi facture brouillon ──────

it('affiche le bouton Ajouter ligne libre ssi facture brouillon', function (): void {
    // Brouillon → bouton présent
    Livewire::test(FactureEdit::class, ['facture' => $this->facture])
        ->assertSee('Ajouter ligne libre');
    ...
    Livewire::test(FactureEdit::class, ['facture' => $this->facture])
        ->assertDontSee('Ajouter ligne libre');
});

// ── Test 2 : Submit ligne libre montant → ligne MontantLibre, montant=2400, total=2400 ──

it('ajouterLigneLibreMontant crée une ligne MontantLibre et recalcule le total', function (): void {
...
it('ajouterLigneLibreTexte crée une ligne Texte sans modifier le total', function (): void {
    // D'abord, ajouter une ligne MontantLibre pour avoir un total de référence
...
it('ajouterLigneLibreMontant avec PU=0 affiche une erreur et ne crée pas de ligne', function (): void {
    $countBefore = FactureLigne::count();
...
// ── Test 6 : Facture validée → boutons "Ajouter ligne libre" et "Ajouter ligne texte" absents ──

it('une facture validée ne montre ni le bouton Ajouter ligne libre ni Ajouter ligne texte', function (): void {
    $this->facture->update([
        'statut' => StatutFacture::Validee,
...
// ── Test 7 : Mix Montant ref + MontantLibre + Texte → lignes des 3 types, total cohérent ──

it('mix Montant ref + MontantLibre + Texte donne les 3 types et un total cohérent', function (): void {
    $compte = CompteBancaire::factory()->create(['association_id' => $this->association->id]);
    $transaction = Transaction::factory()->asRecette()->create([
        'association_id' => $this->association->id,
        'tiers_id' => $this->tiers->id,
    ...
    expect($lignes->where('type', TypeLigneFacture::MontantLibre)->count())->toBe(1);
    expect($lignes->where('type', TypeLigneFacture::Texte)->count())->toBe(1);

    // Total = somme des montants non-null (Montant ref + MontantLibre 100)
    $this->facture->refresh();
