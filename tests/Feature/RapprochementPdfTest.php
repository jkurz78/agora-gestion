...
it('requires authentication to download PDF', function () {
    $this->get(route('compta.rapprochement.pdf', $this->rapprochement))
        ->assertRedirect(route('login'));
});

it('returns 404 for non-existent rapprochement', function () {
    $this->actingAs($this->user)
        ->get(route('compta.rapprochement.pdf', 99999))
        ->assertNotFound();
});
...
it('ouvre le PDF inline avec ?mode=inline', function () {
    $response = $this->actingAs($this->user)
        ->get(route('compta.rapprochement.pdf', $this->rapprochement).'?mode=inline');
