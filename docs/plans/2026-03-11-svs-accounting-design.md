...
### `depense_lignes` *(ventilation analytique)*
| Colonne | Type | Notes |
|---------|------|-------|
| id | INT PK AUTO_INCREMENT | |
| depense_id | INT FK | |
...
### `recette_lignes` *(ventilation analytique)*
| Colonne | Type | Notes |
|---------|------|-------|
| id | INT PK AUTO_INCREMENT | |
| recette_id | INT FK | |
...
- Filtre exercice *(obligatoire)*
- Filtre opérations : "Toutes" *(défaut)* ou sélection multiple
- Présentation charges/produits agrégés par `code_cerfa`
- Export CSV

#### Rapport par séances
- Sélection de l'opération *(filtrée sur celles avec `nombre_seances` renseigné)*
