...
L'application tourne sur un hébergement mutualisé O2Switch (cPanel) sous le sous-domaine `***DEPLOY_SUBDOMAIN***`. Le déploiement est actuellement manuel (FTP + commandes SSH dans le terminal cPanel). L'objectif est d'automatiser le déploiement à chaque push sur `main`, avec exécution des tests avant toute mise en production.
