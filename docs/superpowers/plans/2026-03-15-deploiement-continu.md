...
```bash
echo "DEPLOY_SECRET=<le token de O1>" >> ~/public_html/***DEPLOY_SUBDOMAIN***/.env
```
...
curl --fail -v \
  ...
  https://***DEPLOY_SUBDOMAIN***/deploy.php
```

Résultat attendu : code `200` + `{"status":"ok"}` dans la réponse.
Vérifier aussi le log :
```bash
tail -50 ~/public_html/***DEPLOY_SUBDOMAIN***/deploy.log
```
...
2. Vérifier sur O2Switch :
   ```bash
   tail -30 ~/public_html/***DEPLOY_SUBDOMAIN***/deploy.log
   ```
3. Charger `https://***DEPLOY_SUBDOMAIN***` et vérifier que l'app répond normalement
